/**************************************************************************************************
 *
 *      Standard procedures - 10Apr21
 *      
 *      header is modified for DadsHouseMonitor to prevent wifi triggering radar
 *      
...
String decodeIP(String IPadrs) {
    ...
    if (IPadrs == "192.168.1.111") IPadrs = "main pc";
    else if (IPadrs == "192.168.1.112") IPadrs = "phone";
...
void webheader(WiFiClient &client, char style[] = " ", int refresh = 0) {
  ...
  // reset radar sensor as wifi can give false triggers
    lastMovementPinState = 0;
    lastWebAccess = millis();      // log time of last web access so radar false triggers can be ignored
