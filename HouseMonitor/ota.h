...
void handleOTA(){
  ...
  if (OTAEnabled == 1) {
    ...
    // OTA is enabled so implement it
      ...
      client.println("<br><H1>Update firmware</H1><br>");
      client.printf("Current version =  %s, %s ", stitle, sversion);
      
      client.println("<form method='POST' action='/update' enctype='multipart/form-data'>");
      client.println("<input type='file' style='width: 300px' name='update'>");
      client.println("<br><br><input type='submit' value='Update'></form><br>");
    
      client.println("<br><br>Device will reboot when upload complete");
